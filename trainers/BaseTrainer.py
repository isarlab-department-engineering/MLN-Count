    ...
    def train(self):
        ...
        for epoch in range(self.opt['start_epoch'] + 1, self.opt['max_epochs'] + 1):
            ...
            for i, data in enumerate(self.trainloader, 0):
                ...
                # if total_steps % self.opt['display_freq'] * self.opt['batch_size'] == 0:
                #     self.model.log_visuals(self.writer, total_steps)
                # visuals = self.model.get_current_visuals()
                # for label, image in visuals.items():
                #     self.writer.add_image(label, image[0], total_steps)
