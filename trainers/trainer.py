...
class PresenceAbsenceTrainer(BaseTrainer):
    ...
    def train(self):
        ...
        for epoch in range(self.opt['start_epoch'] + 1, self.opt['max_epochs'] + 1):
            ...
            self.writer.add_scalar('epoch_train_loss/', latest_loss, epoch)

            if epoch % self.opt['save_epoch_freq'] == 0 and epoch != 0:
                if self.opt['resume_train'] and epoch == self.opt['start_epoch']:
                    pass
                else:
    ...
    def val(self, epoch):
        ...
        self.writer.add_scalar('epoch_val_loss/', latest_loss, epoch)
